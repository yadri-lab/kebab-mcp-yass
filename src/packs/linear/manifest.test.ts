...
import { linearPack } from "./manifest";

describe("linearPack manifest", () => {
  it("registers all 6 tools with correct names", () => {
    const names = linearPack.tools.map((t) => t.name);
  ...
  it("marks write tools as destructive", () => {
    const destructiveTools = linearPack.tools.filter((t) => t.destructive).map((t) => t.name);
  ...
  it("does not mark read tools as destructive", () => {
    ...
    for (const name of readToolNames) {
      const tool = linearPack.tools.find((t) => t.name === name);
      expect(tool?.destructive).toBeFalsy();
    }
  });

  it("requires LINEAR_API_KEY env var", () => {
    expect(linearPack.requiredEnvVars).toContain("LINEAR_API_KEY");
  });

  it("has id 'linear'", () => {
    expect(linearPack.id).toBe("linear");
  });
});
